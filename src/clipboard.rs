// Vendored from bevy_mod_clipboard - tired of waiting for people to bump bevy versions on their crates :(
// https://github.com/ickshonpe/bevy_mod_clipboard
...
/// Bevy plugin that adds clipboard support.
...
impl ClipboardRead {
    ...
    ///
    /// Subsequent calls after the contents have been read, return `Some(ClipboardError::ContentTaken)`.
...
impl Clipboard {
    ...
    pub fn fetch_text(&mut self) -> ClipboardRead {
        ...
        #[cfg(not(any(target_family = "unix", windows, target_arch = "wasm32")))]
        {
            ClipboardRead::Ready(Err(ClipboardError::ClipboardNotSupported))
        }
    }
...
#[cfg(any(windows, target_family = "unix"))]
